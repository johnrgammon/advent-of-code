...
public class Day1 {
    ...
    private final static int EXPECTED_RESULT_DEMO_PART_ONE = 142;
    private final static String PUZZLE_INPUT_DEMO_PART_ONE;
    private final static int EXPECTED_RESULT_DEMO_PART_TWO = 281;
    private final static int EXPECTED_RESULT_PART_ONE = 55017;
    private final static int EXPECTED_RESULT_PART_TWO = 53539;
    private final static String PUZZLE_INPUT;
    private final static String PUZZLE_INPUT_DEMO_PART_TWO;
    static {
        //region demo_data
        PUZZLE_INPUT_DEMO_PART_ONE = """
                1abc2
                pqr3stu8vwx
                a1b2c3d4e5f
                treb7uchet""";
        PUZZLE_INPUT_DEMO_PART_TWO = """
                two1nine
                eightwothree
                abcone2threexyz
                xtwone3four
    ...
    private static String findFirstDigitWord(String dataStr) {
        TreeMap<Integer, String> firstOccurrences = CONVERSION_MAP.keySet()
                .stream()
                .filter(dataStr::contains)
                .collect(Collectors.toMap(
                        dataStr::indexOf,
                        Function.identity(),
                        (o, n) -> n,
                        TreeMap::new));
        return firstOccurrences.isEmpty() ? null : firstOccurrences.firstEntry().getValue();
    }

    private static String findLastDigitWord(String dataStr) {
        TreeMap<Integer, String> lastOccurrences = CONVERSION_MAP.keySet()
                .stream()
                .filter(dataStr::contains)
                .collect(Collectors.toMap(
                        dataStr::lastIndexOf,
                        ...
                        (o, n) -> n,
                        TreeMap::new));
        return lastOccurrences.isEmpty() ? null : lastOccurrences.lastEntry().getValue();
    }

    private static int findFirstDigitIndex(String string) {
        Matcher matcher = DIGIT_PATTERN.matcher(string);
        return matcher.find() ? matcher.start() : -1;
    }

    private static int findLastDigitIndex(String string) {
        Matcher matcher = DIGIT_PATTERN.matcher(string);
    ...
    public static void main(Object... args) {
        ...
        if (part == 1) {
            dataStr = isDemo ? PUZZLE_INPUT_DEMO_PART_ONE : PUZZLE_INPUT;
            expectedCalibration = isDemo ? EXPECTED_RESULT_DEMO_PART_ONE : EXPECTED_RESULT_PART_ONE;

            coordinates = Arrays.stream(dataStr.split("\n"))
                    .map(input -> input.replaceAll("\\D+", ""))
                    .map(input -> input.length() == 1 ? input + input : input)
                    .map(input -> input.length() > 2 ? input.charAt(0) + "" + input.charAt(input.length() - 1) : input)
                    .map(Integer::parseInt)
                    .collect(Collectors.toList());
        }
        else {
            dataStr = isDemo ? PUZZLE_INPUT_DEMO_PART_TWO : PUZZLE_INPUT;
            expectedCalibration = isDemo ? EXPECTED_RESULT_DEMO_PART_TWO : EXPECTED_RESULT_PART_TWO;

            coordinates = Arrays.stream(dataStr.split("\n"))
                    .map(row -> {
                        String coordinate = "";

                        String firstDigitWord = findFirstDigitWord(row);
                        int firstDigitIndex = findFirstDigitIndex(row);
                        if (firstDigitWord != null && (row.indexOf(firstDigitWord) < firstDigitIndex || firstDigitIndex == -1)) {
                            coordinate = coordinate.concat(CONVERSION_MAP.get(firstDigitWord));
                        } else {
                            coordinate = coordinate.concat(String.valueOf(row.charAt(firstDigitIndex)));
                        }

                        String lastDigitWord = findLastDigitWord(row);
                        int lastDigitIndex = findLastDigitIndex(row);
                        if (lastDigitWord != null && row.lastIndexOf(lastDigitWord) > lastDigitIndex) {
                            coordinate = coordinate.concat(CONVERSION_MAP.get(lastDigitWord));
                        } else {
                            coordinate = coordinate.concat(String.valueOf(row.charAt(lastDigitIndex)));
                        }

                        return coordinate.isBlank() ? 0 : Integer.parseInt(coordinate);
                    }).collect(Collectors.toList());
        }

        int calibration = coordinates.stream().reduce(0, Integer::sum);
        Utils.publishResult(startTime, expectedCalibration, calibration);
    }
}
