...
public class Day2 {
    ...
    private final static int EXPECTED_ID_SUM_DEMO = 8;
    private final static int EXPECTED_ID_SUM = 2776;
    private final static int EXPECTED_POWER_SUM_DEMO = 2286;
    private final static int EXPECTED_POWER_SUM = 68638;

    private final static String GAMES_DEMO;
    private final static String GAMES;
    static {
        //region demo_data
        GAMES_DEMO = """
    ...
    private static Integer getRoundColourCount(String colour, String round) {
        return Integer.parseInt(Arrays.stream(round.trim().split(", "))
                .filter(cubeDef -> cubeDef.contains(colour))
                .findFirst()
                .orElse("0")
                .replace(" " + colour, ""));
    }

    private static Boolean isColourViolation(String colour, String round) {
        Map<String, Integer> ADVENT_TWO_MAXES = Map.of(
                COLOUR_BLUE, 14,
                COLOUR_GREEN, 13,
                COLOUR_RED, 12);

        int colourCount = getRoundColourCount(colour, round);
        return ADVENT_TWO_MAXES.containsKey(colour) && colourCount > ADVENT_TWO_MAXES.get(colour);
    }

    public static void main(int part, boolean isDemo) {
        String dataStr = isDemo ? GAMES_DEMO : GAMES;
        Map<Integer, List<String>> gameIdToRounds = Arrays.stream(dataStr.split("\n"))
                .collect(Collectors.toMap(
                        s -> Integer.parseInt(s.split(": ")[0].replace("Game ", "")),
                        s -> Arrays.asList(s.split(": ")[1].split("; "))));
        int actualResult;
        int expectedResult;
        ...
        if (part == 1) {
            ...
            gameIdToRounds.forEach((gameId, rounds) -> {
                boolean gameIsValid = true;
                for (String round : rounds) {
                    if (isColourViolation(COLOUR_BLUE, round)
                            || isColourViolation(COLOUR_GREEN, round)
                            || isColourViolation(COLOUR_RED, round)) {
                        gameIsValid = false;
                        break;
                    }
                }
            ...
            actualResult = validGameIds.stream().reduce(0, Integer::sum);
            expectedResult = isDemo ? EXPECTED_ID_SUM_DEMO : EXPECTED_ID_SUM;
        } else {
            List<Integer> gamePowerValues = new ArrayList<>();
            gameIdToRounds.forEach((gameId, rounds) -> {
                int blueMin = 0;
                int greenMin = 0;
                int redMin = 0;
                for (String round : rounds) {
                    blueMin = Math.max(blueMin, getRoundColourCount(COLOUR_BLUE, round));
                    greenMin = Math.max(greenMin, getRoundColourCount(COLOUR_GREEN, round));
                    redMin = Math.max(redMin, getRoundColourCount(COLOUR_RED, round));
                }
                gamePowerValues.add(blueMin * greenMin * redMin);
            });

            actualResult = gamePowerValues.stream().reduce(0, Integer::sum);
            expectedResult = isDemo ? EXPECTED_POWER_SUM_DEMO : EXPECTED_POWER_SUM;
        }
