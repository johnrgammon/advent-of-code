...
public class Day3 {
    ...
    private static final int EXPECTED_PART_NUMBER_SUM_DEMO = 4361;
    private static final int EXPECTED_GEAR_RATIO_SUM_DEMO = 467835;
    private static final int EXPECTED_PART_NUMBER_SUM = 530849;
    private static final int EXPECTED_GEAR_RATIO_SUM = 84900879;
    private static final String ENGINE_SCHEMATIC_DEMO;
    private static final String ENGINE_SCHEMATIC;
    static {
        //region demo_data
        ENGINE_SCHEMATIC_DEMO = """
                467..114..
                ...*......
                ..35..633.
                ......#...
    ...
    public static void main(Integer part, boolean isDemo) {
        String dataStr = isDemo ? ENGINE_SCHEMATIC_DEMO : ENGINE_SCHEMATIC;
        List<String> rows = Arrays.asList(dataStr.split("\n"));
        List<Integer> validNumbers = new ArrayList<>();
        List<Integer> validRatios = new ArrayList<>();
        Map<String, List<String>> coordinatesMap = new HashMap<>(); // R+row#+P+pos# to list of numbers
        int actualResult;
        int expectedResult;
        ...
        rows.forEach(row -> {
            ...
            String rowAbove = rowIndex > 0 ? rows.get(rowIndex - 1) : null;
            String rowBelow = rowIndex < rows.size() - 1 ? rows.get(rowIndex + 1) : null;
            if (row.chars().anyMatch((Character::isDigit))) {
                String numberStr = "";
                ...
                for (Character c : row.toCharArray()) {
                    if (Character.isDigit(c)) {
                        numberStr = numberStr.concat(Character.toString(c));
                        if (startIndex == null) {
                            startIndex = charIndex;
                        }
                        if (charIndex == row.length() - 1 || !Character.isDigit(row.charAt(charIndex + 1))) {
                            stopIndex = charIndex;
                        }
                    }
                    ...
                    if (startIndex != null && stopIndex != null) {
                        ...
                        if (startIndex > 0) {
                            checkPositions.add(startIndex - 1);
                        }

                        if (stopIndex < row.length() - 1) {
                            checkPositions.add(stopIndex + 1);
                        }

                        for (int i = startIndex; i <= stopIndex; i++) {
                            checkPositions.add(i);
                        }

                        List<Character> potentialSymbols = new ArrayList<>();
                        String coord = null;
                        if (rowAbove != null) {
                            for (int i : checkPositions) {
                                if (rowAbove.charAt(i) == '*') {
                                    coord = "r" + rows.indexOf(rowAbove) + "p" + i;
                                }
                                potentialSymbols.add(rowAbove.charAt(i));
                            }
                        }
                        for (int i : checkPositions) {
                            if (row.charAt(i) == '*') {
                                coord = "r" + rows.indexOf(row) + "p" + i;
                            }
                            potentialSymbols.add(row.charAt(i));
                        }
                        if (rowBelow != null) {
                            for (int i : checkPositions) {
                                if (rowBelow.charAt(i) == '*') {
                                    coord = "r" + rows.indexOf(rowBelow) + "p" + i;
                                }
                                potentialSymbols.add(rowBelow.charAt(i));
                            }
                        }
                        if (coord != null) {
                            if (coordinatesMap.containsKey(coord)) {
                                coordinatesMap.get(coord).add(numberStr);
                            } else {
                                coordinatesMap.put(coord ,new ArrayList<>(List.of(numberStr)));
                            }
                        }

                        if (potentialSymbols.stream().anyMatch(character -> character != '.'
                                && !Character.isLetter(character)
                                && !Character.isDigit(character))) {

                            validNumbers.add(Integer.parseInt(numberStr));
                        }
        ...
        coordinatesMap.forEach((key, value) -> {
            if (value.size() == 2) {
                validRatios.add(Integer.parseInt(value.get(0)) * Integer.parseInt(value.get(1)));
            }
        });

        if (part == 1) {
            actualResult = validNumbers.stream().reduce(0, Integer::sum);
            expectedResult = isDemo ? EXPECTED_PART_NUMBER_SUM_DEMO : EXPECTED_PART_NUMBER_SUM;
        } else {
            actualResult = validRatios.stream().reduce(0, Integer::sum);
            expectedResult = isDemo ? EXPECTED_GEAR_RATIO_SUM_DEMO : EXPECTED_GEAR_RATIO_SUM;
        }

        Utils.publishResult(stopTime, expectedResult, actualResult);
    }
}
